...
@stop
@section('menu')
Referensi > Wilayah > <b>Kecamatan</b>
@stop
<!--begin::Col-->
    ...
    <div class="col-lg-{{$isOpen ? '8' : '12' }} col-xxl-{{$isOpen ? '8' : '12' }}">
        ...
        <div class="card card-flush h-md-100">
            ...
            <div class="card-header align-items-center gap-2 gap-md-5">
                ...
                @if(!is_null($kabupaten_list))
                    <div class="w-200 mw-350px" wire:ignore.self >
                        <select x-init="$($el).select2({ placeholder: '-- Pilih Kabupaten --', });
                        $($el).on('change', function() {
                            $wire.set('searchKabupaten', $($el).val());
                        })" wire:model.live="searchKabupaten"  name="searchKabupaten" id="searchKabupaten" class="form-control form-control-lg form-select-solid">
                    ...
                    <table class="table table-row-dashed align-middle gs-0 gy-3 my-0">
                        ...
                        <thead>
                            <tr class="text-uppercase">
                                <th class="pl-0" style="min-width: 100px">Kode Kecamatan
                                </th>
                    ...
                    <div class="card-body pt-0">
                        ...
                        <div class="mb-3 fv-row fv-plugins-icon-container" wire:ignore>
                            ...
                            @if ($mode=='create' ) 
                                <select x-init="$($el).select2({ placeholder: '-- Pilih Kabupaten --', });
                                $($el).on('change', function() {
                                    $wire.set('id_kab', $($el).val())
                                })" wire:model.live="id_kab"  name="id_kab" id="id_kab" class="form-control form-control-lg form-select-solid">
                                <option value=""></option>
                                    @foreach($kabupaten_list as $kabupaten)
                                    <option value="{{$kabupaten->id}}">{{$kabupaten->name}}</option>
                                    @endforeach
                            ...
                            @else
                                ...
                                $($el).on('change', function() {
                                    $wire.set('id_kab', $($el).val())
                                })" wire:model.live="id_kab"  name="id_kab" id="id_kab" disabled class="form-control form-control-lg form-select-solid">
                                <option value=""></option>
                                    @foreach($kabupaten_list as $kabupaten)
                                    <option value="{{$kabupaten->id}}">{{$kabupaten->name}}</option>
                                    @endforeach
                        ...
                        <div class="mb-3 fv-row fv-plugins-icon-container">
                            <label class="required form-label">Kode Kecamatan</label>
                            @if ($mode=='create' ) 
                            <input type="text" class="form-control  @error('id_kec') is-invalid @enderror"
                                placeholder="Ex: 320410" wire:model="id_kec" />
                            @else
                            <input type="text" disabled class="form-control  @error('id_kec') is-invalid @enderror"
                                placeholder="Ex: 320410" wire:model="id_kec" />
                            @endif
                            @error('id_kec')
                            <div
                        ...
                        <div class="mb-3 fv-row fv-plugins-icon-container">
                            <label class="required form-label">Nama Kecamatan</label>
                            <input type="text" class="form-control  @error('name') is-invalid @enderror"
                                placeholder="Ex: Arjasari" wire:model="name" />
                            @error('name')
                            <div
                                class="fv-plugins-message-container fv-plugins-message-container--enabled invalid-feedback">
                                {{ $message }} </div>
