            ...
            <table class="table table-head-custom table-vertical-center table-hover table-striped" id="kt_advance_table_widget_2">
                <thead>
                    <tr class="text-uppercase">
                        ...
                        <th style="min-width: 120px text-center">Satuan Kerja
                        @if ($sortColoumName === 'satuan_kerja')
                            <span wire:click="sortBy('satuan_kerja')" class="float-end text-sm" style="cursor: pointer;">
                                <i class="{{ $sortColoumName === 'satuan_kerja' && $sortDirection === 'desc' ? ' fas fa-sort-up ' : 'fas fa-sort-down' }}"></i>
                            </span> 
                        @else
                            <span wire:click="sortBy('satuan_kerja')" class="float-end text-sm" style="cursor: pointer;">
                                <i class="fas fa-sort"></i>
                ...
                <tbody>
                    @foreach ($model as $index => $item)
                        <tr>
                            ...
                            <td>
                                <span class="text-center text-gray-80 font-weight-bolder text-hover-success font-size-lg">{{ $item->satuan_kerja}}</span>
                            </td>
    ...
    <form action="">
        <div>
            <div class="form-group">
                ...
                <input type="hidden" class="form-control  @error('satuan_kerja_id') is-invalid @enderror" wire:model="satuan_kerja_id" />
                @error('kode')
            ...
            <div class="form-group">
                <label>Satuan Kerja
                <span class="text-danger">*</span></label>
                <input type="text" class="form-control  @error('satuan_kerja') is-invalid @enderror" placeholder="Ex: Jamkrida Jawa Barat" wire:model="satuan_kerja" />
                @error('satuan_kerja')
                    <div class="invalid-feedback form-text text-danger"> {{ $message }} </div>
                @enderror
            </div>                                                
