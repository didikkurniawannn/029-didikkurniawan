            ...
            <table class="table table-head-custom table-vertical-center table-hover table-striped" id="kt_advance_table_widget_2">
                <thead>
                    <tr class="text-uppercase">
                        ...
                        <th style="min-width: 120px text-center">Jabatan
                        @if ($sortColoumName === 'jabatan')
                            <span wire:click="sortBy('jabatan')" class="float-end text-sm" style="cursor: pointer;">
                                <i class="{{ $sortColoumName === 'jabatan' && $sortDirection === 'desc' ? ' fas fa-sort-up ' : 'fas fa-sort-down' }}"></i>
                            </span> 
                        @else
                            <span wire:click="sortBy('jabatan')" class="float-end text-sm" style="cursor: pointer;">
                                <i class="fas fa-sort"></i>
                        ...
                        <th class="pr-0 text-right" style="min-width: 160px">Status Jabatan</th>
                        <th class="pr-0 text-right" style="min-width: 160px">Status PJ</th>
                        <th class="pr-0 text-right" style="min-width: 160px">Status Tujuan</th>
                ...
                <tbody>
                    @foreach ($model as $index => $item)
                        <tr>
                            ...
                            <td>
                                <span class="text-center text-gray-80 font-weight-bolder text-hover-success font-size-lg">{{ $item->jabatan}}</span>
                            </td>
                            <td class="pr-0 text-right">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckStatus{{ $item->id }}" wire:click="statusJabatan({{ $item->id }})" {{ $item->status_jabatan == 0 ? '' : 'checked' }}>
                                    <label class="form-check-label" for="flexSwitchCheckStatus{{ $item->id }}">{{ $item->status_jabatan == 0 ? 'Non Struktural' : 'Struktural' }}</label>
                                </div>
                            </td>
                            <td class="pr-0 text-right">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckStatus{{ $item->id }}" wire:click="statusPejabat({{ $item->id }})" {{ $item->status_pj == 0 ? '' : 'checked' }}>
                                    <label class="form-check-label" for="flexSwitchCheckStatus{{ $item->id }}">{{ $item->status_pj == 0 ? 'Tidak Aktif' : 'Aktif' }}</label>
                                </div>
                            </td>
                            <td class="pr-0 text-right">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckStatus{{ $item->id }}" wire:click="statusTujuan({{ $item->id }})" {{ $item->status_tujuan == 0 ? '' : 'checked' }}>
                                    <label class="form-check-label" for="flexSwitchCheckStatus{{ $item->id }}">{{ $item->status_tujuan == 0 ? 'Tidak Aktif' : 'Aktif' }}</label>
                                </div>
                            </td>                            
                            <td class="pr-0 text-right">
                                <div class="form-check form-switch">
        ...
        <div>
            ...
            <div class="form-group">
                ...
                <span class="text-danger">*</span></label>
                    ...
                    @foreach ($jabatan_list_atasan as $item)
